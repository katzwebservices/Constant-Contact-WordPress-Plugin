...
"></i> ' . __( 'Comment Form', 'constant-contact-api' ), '<h3>' . esc_html__( 'Configure how users sign up.', 'constant-contact-api' ) . '</h3>', 'constant-contact-api' );
		...
		$groups = array(
			'setup'        => array(
				...
				array(
					'id'    => 'eventspot',
					'type'  => 'checkbox',
					'desc'  => __( 'Enable EventSpot Integration', 'constant-contact-api' ),
					'label' => __( 'Enable integration with Constant Contact Events (EventSpot)?', 'constant-contact-api' ),
				),
				array(
					'type'    => 'checkboxes',
					'id'      => 'logging',
		...
		foreach ( $groups as $group => $settings ) {
			foreach ( $settings as $setting ) {
				...
				$setting['callback'] = isset( $setting['callback'] ) ? $setting['callback'] : array(
					'CTCT_Settings',
					'setting_input_generator'
				);
	...
	static function setting_input_generator( $settings ) {
		...
		switch ( $type ) {
			...
			case "radio":
				...
				if ( in_array( $type, array( 'lists', 'checkboxes' ) ) ) {
					$type = 'checkbox';
					$name .= '[]';
				} else {
	...
	static function do_settings_sections( $page ) {
		...
		<p class="submit"><input name="Submit" id="ctct-save-settings" class="button button-primary button-hero"
		                         type="submit" value="<?php esc_html_e( 'Save Settings', 'constant-contact-api' ); ?>"/>
		</p>
		<?php
	}
