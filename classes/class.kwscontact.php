...
class KWSContact extends Contact {
	...
	private function prepareName( $contact_array, $key = 'name' ) {
		...
		if ( isset( $name['middle'] ) && empty( $contact_array['middle_name'] ) ) {
			$contact_array['middle_name'] = $name['middle'];
		}

		if ( isset( $name['last'] ) && empty( $contact_array['last_name'] ) ) {
			$contact_array['last_name'] = $name['last'];
		}
	...
	function getAddress( $type = 'personal', $format = false ) {
		$output = '';
		foreach ( $this->addresses as $Address ) {
			if ( strtoupper( $type ) !== $Address->address_type ) {
				continue;
			}
