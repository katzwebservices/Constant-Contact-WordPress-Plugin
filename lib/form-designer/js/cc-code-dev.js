...
//		$("#settings:not(.formfields input,.formfields textarea)").bind('change', function() {  updateCode(false, false, '#settings'); });
	...
	function updateWidthCalculator() {
		...
		var setwidth = Math.floor(rawwidth) - Math.floor(paddingwidth) - Math.floor(borderwidth);
		var realwidth = Math.floor(rawwidth) + Math.floor(paddingwidth) + Math.floor(borderwidth);
			$('label[for=width] span.ctct_qtip').attr('title', '<p><strong>Actual width is '+ realwidth + 'px.</strong> <em>For an form that is '+rawwidth+'px wide, set Form Width to '+setwidth+'px</em></p>').qtip({
		style: {
			...
			$('label[for=width] span.ctct_qtip').attr('title', '<p><strong>Width depends on the size of the container.</strong> For example, for a form in a sidebar, the form\'s width percentage will be based on the width of the sidebar.</p>');
		}
	}
